...
| Cada jugador que se unía veía otro reloj | Un plazo sincronizado es un número de cuadro del contador del host (`gGlobalTimer`, que empieza en cero al abrir cada juego y no viaja por la red), y cada máquina lo restaba de su propio contador, así que un cliente se equivocaba en toda la ventaja que llevaba el juego del host | `SH.host_timer()` en `modules/core.lua`: el host publica su contador en `sh5_host_timer` una vez por segundo y cada cliente guarda la diferencia en cuanto llega un valor nuevo. **Un plazo sincronizado se lee contra `SH.host_timer()`, nunca contra `get_global_timer()`**; el código del host y todo lo que no sale de una máquina sí usan `get_global_timer()`, que es lo que significa | `test/suite/clock.lua`; y `test/live/run.sh`, donde cada cliente imprime `PROBE clock` y `--without r035` lo pone en rojo |
