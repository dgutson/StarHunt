...
| Actos y PvP | Pide al motor que ignore el acto entre jugadores, y responde si dos están en el mismo sitio | `modules/core.lua` (`enable_cross_act_players`); `modules/goals.lua` (`players_can_share_world`); `modules/team.lua` (`on_allow_pvp_attack`) |
...
| Cada jugador veía un reloj distinto | Los plazos vivían en las tablas sincronizadas como números de cuadro del contador del host (`gGlobalTimer`, que empieza en cero al abrir cada juego, corre a 60 por segundo en la pantalla de carga y nunca recupera lo que pierde un tirón), y cada máquina los restaba de su propio contador | Ninguna cuenta atrás se sincroniza. Cada máquina cuenta la suya en segundos reales con `clock_elapsed()`, desde la marca que puso cuando vio empezar la cuenta. Todo el mecanismo son `SH.watch_clock`, `SH.seconds_left(name)` y `SH.set_clock_remaining` en `modules/core.lua`, y los relojes se declaran en el cuerpo de `main.lua` porque `hook_on_sync_table_change` solo se registra durante la carga. Escribir una clave con el valor que ya tenía reinicia la cuenta en todas las máquinas: el motor no compara (`src/pc/lua/smlua_sync_table.c:294` y `:433`). Ningún módulo más tiene código de reloj, y el host no tiene marcas propias. **Lo que viaja por la red es el evento, nunca el tiempo**: `sh5_round`, `sh5_manual_reroll_seq` y `sh5_chaos_modifier_seq`. `get_global_timer()` solo vale para lo que de verdad se mide en cuadros | `test/suite/clock.lua`; y `test/live/run.sh`, donde los contadores de cuadros de los dos clientes se diferencian en cientos de cuadros y las cuentas atrás coinciden, con `--without r035` para ponerlo en rojo |
| Morir reiniciaba la espera de OTRO NIVEL | `host_assign_goal` fijaba el plazo en cada reasignación que hacía, incluida la que pide la rama de forfeit tras una muerte | El plazo lo fijan los dos llamadores que lo quieren decir: `host_prepare_player`, cuando el jugador entra a la ronda, y el propio botón. Una meta entregada por una muerte o por terminar una estrella no lo toca | `test/suite/round_host.lua`: qué empieza la espera y qué la deja donde estaba |
...
| Dos jugadores del mismo curso con actos distintos no se veían ni se tocaban | El acto hace dos trabajos en el motor: decide qué objetos aparecen y decide si otro jugador cuenta como presente. `is_player_active` (`obj_behaviors.c`), `interact_player`, `interact_player_pvp`, el bloque que esconde al Mario remoto (`mario.c`), `network_receive_player` y los nametags comparan `currActNum`, todos **antes** de cualquier hook de Lua | El mod pide una sola vez, al cargar, que el motor deje el acto fuera de esas comparaciones: `enable_cross_act_players` (`modules/core.lua`) escribe `gLevelValues.crossActPlayers`, que existe en las ramas `feature/cross-act-players` y `feature/cross-act-interpolation` de `dgutson/sm64coopdx`. En la segunda el motor además deja de simular localmente el cuerpo de un jugador cuyo acto es distinto y lo coloca donde dice su propio juego, así que ese cuerpo se ve suave; el mod no pide nada para eso. **El mod no vuelve a separar a dos jugadores por su acto**: no hay predicado de mundo privado, ni cajas por zona, ni jugadores ocultos, ni negativa de `INTERACT_PLAYER`. Donde el campo no existe, el motor los separa por su cuenta y el mod lo deja hacer; escribir un campo que el juego no tiene imprime un error de Lua, así que se lee la lista de campos antes | `test/suite/world.lua`: el mod pide el campo cuando existe y no escribe nada cuando no, y ningún acto separa a dos jugadores del mismo curso; el caso `split` del arnés en vivo mide el empujón real entre dos jugadores en actos 6 y 1 |
