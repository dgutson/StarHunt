...
6. Ejecutar la comprobación de sintaxis y la prueba completa:
   ...
   780 pruebas sin fallos, `luacheck StarHunt/ test/` con 2 avisos y 0 errores,
...
| Un modo podia pasar por dificultad o por color de equipo | Los once valores vivian planos en `Team`, asi que `NORMAL`, `EASY` y `NONE` eran 0, y `CHAOS` y `NIGHTMARE` eran 3; ningun test, luacheck ni lua-language-server puede ver ese cambiazo | Tres tablas separadas, `Team.Mode`, `Team.Difficulty` y `Team.TeamColor`, cada una con un metatable que lanza error al leer un nombre de otro eje y que rechaza asignaciones, para que la colision no vuelva campo a campo. **Los numeros no cambiaron**: `sh5_mode` y `sh5_difficulty` van por la red y se usan en aritmetica (`% 4`, `+ 1` como indice, `clamp(..., 0, 3)`), asi que renumerar enfrentaria a un cliente publicado con uno parcheado | `test/suite/core.lua`: los tres ejes son tablas distintas sin ningun nombre en comun, un nombre de otro eje lanza error en vez de responder y el mensaje nombra la clave y apunta a quien la leyo, un eje no gana miembros en ejecucion, cada numero es el que se publico, y los once nombres planos ya no estan en `Team` |
