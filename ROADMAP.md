...
### R-026 — `Team` is two namespaces wearing one name; split it in two

- **Category:** Refactor
- **What:** The table declared in `modules/core.lua:23` holds two unrelated things. Fifteen of its members are genuinely about Team mode — the colour axis, the rosters, the palettes, the per-team score totals and the balancing — and seventy-four are mod-wide and apply in all four modes: the mode and difficulty axes, the language, the HUD drawing helpers, Bowser's health, the modifier readers, the Chaos helpers, the menu and the third-party mod compatibility. Split them: the team-mode members keep the name `Team`, and the rest move to a namespace named for the mod (`SH` or `StarHunt`). Both tables stay declared in `core.lua`, which requires nothing, so every module can reach either without a new `require` edge.
- **Why:** The name is only wrong for part of what it holds, which is why renaming the whole thing would be the wrong fix: `RED`, `BLUE`, the rosters and the palettes really are Team-mode concepts and `Team` is the right word for them. It is everything else that is misfiled, and R-019 made that hard to ignore — `Team.Difficulty.NIGHTMARE` reads as "Team mode's difficulty" and means "the mod's difficulty", and `Team.Mode.CHAOS` reads as a contradiction. `core.lua`'s own header spends a paragraph explaining that the name is wrong, which is the sign that it is.
- **Measured, so the size is not a guess:** 89 distinct members, 556 references. The team-mode side is 15 members and 113 references, which do not move at all; the mod-wide side is 74 members and 443 references, which do. That is fewer edits than renaming the whole table (556) and leaves every name right, so this is both the smaller change and the honest one.
- **The reason it is mechanical:** `Team` is a **local binding** — eleven modules and `main.lua` do `local Team = core.Team`. Each file gains a second line, `local SH = core.SH`, and a substitution over the members that moved. Nothing crosses a file boundary except the two field names on `core`'s return table.
- **Two calls to make before starting:** (1) `Team.is_mode()` is a predicate *about* Team mode but belongs with `selected_mode`, `is_boss_mode` and `is_chaos_mode` on the mod-wide side; splitting the four would be worse than the name. (2) With `Team` meaning Team mode, `Team.TeamColor` stutters — but `Team.Color` collides with the existing `Team.colors`, the palette table, differing only in case and plural. Rename the palette table in the same commit, or keep the stutter.
- **How to do it safely:** one commit that changes nothing but names, proven identical apart from the identifiers, with no mutation sweep because nothing behavioural changes. Every document that names `Team` and every `STARHUNT_TEST_API` key that carries a moved member goes with it.
- **Outcome:** Two namespaces, each named for what it holds; `core.lua`'s header no longer has to correct its own name; the 779 tests pass and the three documented checks stay at their baselines.
- **Blocked-by:** —
- **Enables:** —
