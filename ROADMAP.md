...
- **Engine facts already verified against `coop-deluxe/sm64coopdx`,** so the next session does not have to re-derive them:
  ...
  - **`gNetworkPlayers[i].currActNum` is the act that client loaded, and it is not 0.** `DynOS_Warp_ToLevel` sets `gCurrActStarNum` to the act asked for (`data/dynos_warps.cpp:189`) and `level_update.c:558` publishes it, so after a StarHunt warp it carries the goal's act; the live harness's `split` case reports `my_act=1 their_act=6`. It is 0 in the castle, and it does not follow a goal reassignment until that client's own warp lands, so `sh5_goal` is still what names the *assigned* act.
  - **Two players whose loaded acts differ cannot interact at all, with or without this item.** `is_player_active` (`obj_behaviors.c:542-559`) refuses a remote player whose course, act, level or area differs from the local one, and `network_receive_player` (`packet_player.c:263-270`) drops such a player's position packet and marks it invalid — the `split` case measures both, `active_them=0` and `their_pos_valid=false`. **So suppressing less cannot produce a fight between two players on different acts.** The only pair this item can act on is the one whose loaded acts agree while their goals name different acts: the `hidden`/`ddd`/`wdw` shape, which lasts the 90 frames of `NEXT_GOAL_DELAY` or as long as a player walks back into the other's act. Settle what the item is for against that before measuring a single volume.
...
- **Design:** `ACT_GEOMETRY_PLAN.md` holds the implementation document, its milestones and its engine references. **The mechanism described above cannot deliver the Outcome above,** and that document says why with the engine lines that settle it: `is_player_active` (`src/game/obj_behaviors.c:552-557`) and `interact_player_pvp` (`src/game/interaction.c:1481-1482`) refuse every interaction between two players whose `currActNum` differ, upstream of every Lua hook, and `currActNum` is read-only from Lua. Read that document before writing any code for this item. The Why above still stands; only the volume table and the silhouette are superseded.
- **Blocked-by:** —
- **Enables:** —
