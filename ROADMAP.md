...
- **Why:** The client half is out (see the record in `HISTORY.md`), and the host half is what four other modules are still waiting on. It could not go in the same pass for two measured reasons. First, `host_start_round` **rebinds** `host_used_goals`, `host_seen_done`, `host_seen_forfeit` and `host_player_records`, and `host_end_round` rebinds `host_previous_player_interactions` and `host_previous_pvp_type` — Lua copies a value on `local x = other.x`, so none of those six can be separated from the two functions that rebind them. Second, the host half calls four Boss helpers that are still in `main.lua`: `boss_time_range_for_players`, `boss_has_modifier`, `boss_is_desperate` and `host_read_boss_health_report`. Those belong to `boss.lua`'s second pass, so either they move first or they move along with this.
...
- **Why:** Each of these was left in `main.lua` because it calls into the round loop, and passing those functions in as parameters would hide a real dependency in order to preserve an arbitrary extraction order. Until they move, four modules are misleadingly incomplete — a reader looking for Chaos's round handling finds it in `main.lua`, not in `chaos.lua`. Note that `boss.lua`'s share is also what R-001 is waiting on, so doing boss first may be the cheaper order.
