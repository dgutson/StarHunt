...
-- action 1 is the vanilla thrown update, the second or so of flight at a mine
-- that ends it. HELD_THROWN and HELD_DROPPED are not tested because that same
-- update puts the field back to HELD_FREE on the next frame, so they never
...
local function boss_is_grabbed(bowser)
    return bowser ~= nil and (bowser.oHeldState == HELD_HELD or bowser.oAction == 1)
end
...
local function apply_boss_hazards(m)
    ...
    -- consuming them.
    if bowser == nil then return end
    -- During Bowser's intro, and for as long as a player has hold of him,
    -- consume the current sequence without executing it. The intro case also
    -- protects players who joined after an attack was sent; the grabbed case
