-- StarHunt v1.1.2 - the modifiers, as the local player actually experiences them.
--
...
local function run_static_modifier_checks()
    ...
    for index, goal in ipairs(GOALS) do
        if goal.power ~= nil and goal.power ~= "wing" and goal.power ~= "metal"
            ...
            print("[StarHunt v1.1.2] Invalid required power in slot " .. index .. ".")
            valid = false
        end
        if goal.act == 7 and (goal.level == LEVEL_BOB or goal.level == LEVEL_WF or goal.level == LEVEL_JRB
            ...
            print("[StarHunt v1.1.2] 100-coin goal slipped into slot " .. index .. ".")
            valid = false
        end
        if goal.mods == nil or #goal.mods == 0 then
            print("[StarHunt v1.1.2] Goal " .. index .. " has no modifier choices.")
            valid = false
        else
            for _, modifier_data in ipairs(goal.mods) do
                if not MODIFIER_KINDS[modifier_data.kind] or modifier_data.kind == "auto_crouch" then
                    print("[StarHunt v1.1.2] Invalid modifier: " .. tostring(modifier_data.kind))
                    valid = false
                end
                if modifier_data.kind == "floor_doom" and (modifier_data.value < 4 or modifier_data.value > 9) then
                    print("[StarHunt v1.1.2] Invalid cursed-floor duration: " .. tostring(modifier_data.value))
                    valid = false
                end
            end
        end
    ...
    if math.abs(x1 - x2) > 0.001 or math.abs(z1 - z2) > 0.001 then
        print("[StarHunt v1.1.2] Water-cap safety test failed.")
        valid = false
    end
    ...
    if MODIFIER_AUDIT_COUNTS.checked ~= expected_audits
        ...
        print("[StarHunt v1.1.2] Modifier audit matrix is incomplete.")
        valid = false
    end
    for goal_id = 1, #GOALS do
        for _, template in ipairs(NORMAL_MODIFIER_CATALOG) do
            if MODIFIER_AUDIT[goal_id] == nil or MODIFIER_AUDIT[goal_id][template.kind] == nil then
                print("[StarHunt v1.1.2] Missing audit entry at goal " .. goal_id .. ": " .. template.kind)
                valid = false
            end
        end
    end
    ...
    if swapped ~= B_BUTTON then
        print("[StarHunt v1.1.2] A/B swap safety test failed.")
        valid = false
    end
    local drift_x, drift_y = rotate_stick(32, 0, math.pi * 0.5)
    if math.abs(drift_x) > 0.01 or math.abs(drift_y - 32) > 0.01 then
        print("[StarHunt v1.1.2] Control-drift rotation test failed.")
        valid = false
    end
    if valid then
        print("[StarHunt v1.1.2] 93 goals, 32 modifiers, " .. MODIFIER_AUDIT_COUNTS.checked
