...
| `clock` | the host's frame counter as every machine reads it: the host answering its own counter whatever has been published, the once-a-second publication, a client carrying the difference and keeping its own frames moving between samples, a client that has heard nothing falling back to its own counter, and the four countdowns a client reads against it -- ANOTHER LEVEL, its two-minute end, the HUD's round clock, the menu's status row and the Chaos reroll |
