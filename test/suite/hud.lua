...
return function(t, harness)
    ...
    s.test("the clock reads minutes and zero-padded seconds", function()
        ...
        t.eq(api.format_remaining_time(30 * 5), "0:05", "the leading zero")
        t.eq(api.format_remaining_time(30 * 5 + 29), "0:05", "a part-second is not rounded up")
        t.eq(api.format_remaining_time(30 * 59), "0:59", "one second before the minute")
        t.eq(api.format_remaining_time(30 * 60), "1:00", "exactly one minute")
        t.eq(api.format_remaining_time(30 * 65), "1:05", "a minute and five seconds")
        t.eq(api.format_remaining_time(30 * 754), "12:34", "a long round")
    end)
